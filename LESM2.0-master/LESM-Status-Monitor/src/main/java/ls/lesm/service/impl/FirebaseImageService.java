...
public class FirebaseImageService implements IImageService, ServletContextListener {
	...
	public Object download(int empId) throws FileNotFoundException, IOException {
		EmployeePhoto photo=this.employeePhotoRepo.findByMasterEmployeeDetails(this.masterEmployeeDetailsRepository.findById(empId).get());
...
//	}
	...
	EmployeePhoto photo=this.employeePhotoRepo.findByMasterEmployeeDetails(this.masterEmployeeDetailsRepository.findById(empId).get());
     String fileName=photo.getProfilePic().substring(55);
